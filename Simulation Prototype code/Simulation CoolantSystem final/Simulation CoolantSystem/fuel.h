...
private:
	...
	float coolantTemp;
	float altitude; //altitude is in kilometers, please convert to meters
	float incline;
	bool simCool; //false if we dont want to simulate the cooling system, else we would have this as a true, by default it is false
public:
	void setSpeed(int);
	engine();
	~engine();
